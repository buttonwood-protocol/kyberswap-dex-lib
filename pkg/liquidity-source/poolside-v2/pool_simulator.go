...
func NewPoolSimulator(entityPool entity.Pool) (*PoolSimulator, error) {
	...
	return &PoolSimulator{
		...
		plBps:              uint256.NewInt(uint64(extra.PlBps)),
		feeBps:             uint256.NewInt(uint64(extra.FeeBps)),
		gas:                defaultGas,
...
func (s *PoolSimulator) CalcAmountOut(param poolpkg.CalcAmountOutParams) (*poolpkg.CalcAmountOutResult, error) {
	...
	poolIn, overflow := uint256.FromBig(s.Info.Reserves[indexIn])
	if overflow {
		return nil, ErrInvalidLiquidity
	}

	poolOut, overflow := uint256.FromBig(s.Info.Reserves[indexOut])
	if overflow {
		return nil, ErrInvalidLiquidity
	}
...
func (s *PoolSimulator) getAmountOut(amountIn, poolIn, poolOut *uint256.Int) (amountOut *uint256.Int, err error) {
	...
	if s.plBps.IsZero() {
		adjustedAmountIn := SafeMul(amountIn, SafeSub(uint256.NewInt(10000), s.feeBps))
		adjustedPoolIn := SafeMul(poolIn, uint256.NewInt(10000))
		adjustedPoolOut := SafeMul(poolOut, uint256.NewInt(10000))

		denominator := SafeAdd(adjustedPoolIn, adjustedAmountIn)

		amountOut = SafeDiv(SafeMul(adjustedAmountIn, adjustedPoolOut), denominator)
	} else {
		knOld := kN(poolIn, poolOut, s.plBps)
		...
		s1 := SafeMul(
			new(uint256.Int).Sqrt(
				SafeMul(s.plBps, SafeMul(knOld, knOld)),
			),
			uint256.NewInt(10000),
		)
		knOld = SafeMul(knOld, uint256.NewInt(10000))

		numerator := SafeMul(
			SafeAdd(kDMult(s.plBps, SafeMul(uint256.NewInt(100), adjustedPoolOut)), s1),
			adjustedAmountIn,
		)

		denominator := SafeAdd(
			SafeAdd(
				kDMult(s.plBps, SafeMul(uint256.NewInt(100), adjustedPoolIn)),
				s1,
			),
			kDMult(s.plBps, SafeMul(uint256.NewInt(100), adjustedAmountIn)),
		)
		amountOut = SafeDiv(numerator, denominator)

		if amountOut.Cmp(SafeSub(adjustedPoolOut, uint256.NewInt(1))) > 0 {
			amountOut = SafeSub(adjustedPoolOut, uint256.NewInt(1))
		}

		amountOut = SafeSub(amountOut, new(uint256.Int).Mod(amountOut, uint256.NewInt(10000)))

		knNew := kN(
			SafeAdd(adjustedPoolIn, adjustedAmountIn),
			SafeSub(adjustedPoolOut, amountOut),
			s.plBps,
		)

		if knNew.Cmp(knOld) < 0 {
			rs := Math.MulDivF(knOld, uint256.NewInt(0).Exp(uint256.NewInt(2), uint256.NewInt(112)), knNew)

			exp224 := uint256.NewInt(0).Exp(uint256.NewInt(2), uint256.NewInt(224))
			rAx := SafeSub(SafeMul(rs, rs), exp224)
			rAy := SafeAdd(SafeSub(kDMult(s.plBps, SafeMul(uint256.NewInt(100), adjustedPoolOut)), kDMult(s.plBps, SafeMul(uint256.NewInt(100), amountOut))), s1)
			rAz := SafeMul(kDMult(s.plBps, uint256.NewInt(100)), exp224)

			returnAmount := Math.MulDivF(rAx, rAy, rAz)

			amountOut = SafeSub(amountOut, returnAmount)
		}
	}

	amountOut = SafeDiv(amountOut, uint256.NewInt(10000))
...
func (s *PoolSimulator) GetMetaInfo(_ string, _ string) interface{} {
	return PoolMeta{
		PlBps:       uint16(s.plBps.Uint64()),
		FeeBps:      uint16(s.feeBps.Uint64()),
