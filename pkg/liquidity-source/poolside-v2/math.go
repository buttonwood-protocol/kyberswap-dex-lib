...
var (
	ErrOverflow = errors.New("MULDIV_OVERFLOW")
)
...
func (m *math) MulDivF(x, y, z *uint256.Int) *uint256.Int {
	xy := m.mul512(x, y)
	if xy.Hi.IsZero() {
		return new(uint256.Int).Div(xy.Lo, z)
	}
	...
	if n.Hi.IsZero() {
		return new(uint256.Int).Div(n.Lo, z)
	}
